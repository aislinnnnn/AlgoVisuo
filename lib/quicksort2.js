...
var x = d3.scaleOrdinal().domain(index).range([0, width]),
    a = d3.scaleLinear().domain([0, n - 1]).range([-Math.PI / 4, Math.PI / 4]);

var anotherSvg = d3.select("body").append("svg")
    .attr("width", width + margin.left + margin.right)
    .attr("height", height + margin.top + margin.bottom)
  .append("g")
    .attr("transform", "translate(" + margin.left + "," + (margin.top + height) + ")");

var line = anotherSvg.selectAll("line")
    .data(data)
  .enter().append("line")
...
setInterval(function step() {
  ...
  if (action) switch (action.type) {
    ...
    case "swap": {
      ...
      var t = line._groups[0][action.i];
      line._groups[0][action.i] = line._groups[0][action.j];
      line._groups[0][action.j] = t;
